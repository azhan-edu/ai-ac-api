...
from lib_messaging import add_user_message, chat
...
class PromptEvaluator:
    ...
    def __init__(self, max_concurrent_tasks=3, use_guidelines=False, use_examples=False):
        self.max_concurrent_tasks = max_concurrent_tasks
        self.use_guidelines = use_guidelines
    ...
    def generate_test_case(self, task_description, idea, prompt_inputs_spec={}):
        ...
        test_case = self._parse_json(text)
    ...
    def grade_output(self, test_case, output, extra_criteria):
        ...
        * The full 1-10 scale should be utilized - don't hesitate to give low scores when warranted
        ...
        return self._parse_json(eval_text)

    def run_test_case(self, test_case, run_prompt_function, extra_criteria=None):
        """Run a test case and grade the result"""
        output = run_prompt_function(test_case["prompt_inputs"], use_guidelines=self.use_guidelines, use_examples=self.use_examples)
    ...
    def run_evaluation(
        ...
        with open(self.getpath(json_output_file), "w") as f:
            json.dump(results, f, indent=2)

        html = generate_prompt_evaluation_report(results)
        with open(self.getpath(html_output_file), "w", encoding="utf-8") as f:
            f.write(html)
